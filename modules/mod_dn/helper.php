...
class modDisplayNewsHelper {
	...
	var $version = "DisplayNews by BK 2.0RC1";
	var $css_type = null;
	var $target = "";
   ...
   function mod_automore_out($row)
    ...
    if ($this->params->get('show_more_auto') ) {
      if  ( $this->set_section_id && !is_array($this->set_section_id) && !is_array( $this->set_category_id) )  {
         ...
         $mod_automore_out .= "<div ".modDisplayNewsHelper::dn_style("dn-more").">";
		 ...
		 $mod_automore_out .= modDisplayNewsHelper::create_link( ContentHelperRoute::getCategoryRoute($category->catslug), 
											$this->params->get('text_more'),
											"dn-more",
   ...
   function readmore_out($row, $aroute, $aparams) {
	...
	function cat_out($row) 
      ...
      if ($this->params->get('show_category') ) {
         $cat_out .= "<div ".modDisplayNewsHelper::dn_style("dn-category").">";
         if ($this->params->get('link_category')) {
            if ($row->cat_published == "1") {
			   
			   	$cat_out .= modDisplayNewsHelper::create_link( ContentHelperRoute::getCategoryRoute($row->catslug), 
											$row->cat_title,
											"dn-category",
	...
	function sec_out($row) {
      ...
      if ( $this->params->get('show_section') ) {
         // $sec_out .= "\n\t\t";
         $sec_out .=  "<div ".modDisplayNewsHelper::dn_style("dn-section").">";
                                ...
                                /* if ( $image_scale == 1 ) {
										...
										$text = preg_replace_callback( '@(<img[^>]*\s+)(src\s*=\s*["\']*)([^"\']+)(["\']*)([^>]*>)@i',
                                                                create_function(
                                                                        ...
                                                                        // or alternative escape all $ as \$
                                                                        '$img',
 'return $img[1]." ".modDisplayNewsHelper::imageResize($img[3],'.$this->params->get('image_width').','.$this->params->get('image_height').',"'.$this->params->get('image_scale').'","'.$this->params->get('image_bg', "#FFFFFF").'")." ".$img[5];'
                                                                ),
                                                                $text
                                                        );
                ...
                if ($this->params->get('image') && $this->params->get('image_align')) {
                        ...
                        if ( $this->params->get('image_align') >= 2) {
								$text = preg_replace( '/(<img\s+[^>]*)(\/>)/i', '$1 align="'.$align.'" $2', $text );
                ...
                if ( $this->params->get('image') && $this->params->get('link_image') && !$this->params->get('link_text') ) {
                        ...
                        $text = preg_replace_callback( '/<img[^>]*>/i',
                                                create_function(
                                                        ...
                                                        // or alternative escape all $ as \$
                                                        '$img',
                                                        "return \"<a ".$this->target." href=".JRoute::_(ContentHelperRoute::getArticleRoute($row->slug, $row->catslug, $row->sectionid )).">\".\$img[0].\"<span class='border'></span></a>\";"
                                                ),
                                                $text
                                        );
   ...
   return $author_out;
      ...
      //To show date item created, date mambot called
		 ...
		 if ( ($this->params->get('show_date') ) && (intval( $row->created ) <> NULL) ) {
			 ...
			 $format = $this->params->get('format_date');
		  }

	  if ( $create_date <> null && $format && $this->params->get('show_date') && ( ($this->params->get('show_date') == 1)  || ($aparams->get('show_date', $this->globalConfig->get('show_date'))) ) )  {
		  
			 $date_out .= "<div class='date'>";
			 $date_out .= strftime("".$format ,strtotime($create_date));
			 $date_out .= "</div>";
		  }
   return $date_out;
	  }
 ...
 */
   ...
   // Function that limits title, intro or full to specified character or word length
    function dn_limit( &$text, $limit_type, $length_chars ) {

		if ($length_chars == 0 ) {
			$text = "";
		} else {
			$text = str_replace(array("\r\n", "\n", "\r"), "", $text);
			$text = modDisplayNewsHelper::truncate($text, $length_chars, '', /* $exact = */ $limit_type == 1, /* $considerHtml */ true );
		}
		return;
	...
	//---------------------------------------------------------------------
        ...
        function dn_style( $id, $extra_style='',  $k = "" ) {
                ...
                return "class='${dnstyle}".$this->params->get('moduleclass_sfx')." ".$extra_style."'";
        }

function imageResize($originalImage,$toWidth,$toHeight,$image_scale, $bgcolor){
	...
	if ( $image_scale != "scale" && $multithumb_loaded == 1 ) {
		$imgtemp = plgContentMultithumb::botmt_thumbnail("$originalImage", $toWidth, $toHeight, $image_scale, hexdec($bgcolor) , /* $watermark = */ 0, /* $dest_folder = */ 'thumbs', /* $size_only =  */ 0, /* $size = */ 0, "PNG" );
...
if ($this->params->get( 'set_date_month') != "") {
   if ($this->params->get( 'set_date_month') == "0") {
      $this->params->set( 'set_date_month', date( "m", time()+$tzoffset*60*60 ) );
   }
}
//---------------------------------------------------------------------
...
if ($this->params->get('set_date_year') != "") {
   if ($this->params->get( 'set_date_year') == 0) {
      $this->params->set( 'set_date_year', date( "Y", time()+$tzoffset*60*60 ) );
   }
}
...
//   .                                                            "\n AND (a.sectionid=0 OR d.published = '1')"
   ...
   . ($this->params->get( 'set_article_id') ?                                         "\n  AND (a.id IN (".implode(",", $this->params->get( 'set_article_id')).") )" : '')
   ...
   . ($this->params->get( 'set_date_newly' ) ?                                 "\n  AND (TO_DAYS(ADDDATE(".$db->Quote($now).", INTERVAL $tzoffset HOUR)) - TO_DAYS(ADDDATE(".$this->created.", INTERVAL $tzoffset HOUR)) <= '".$this->params->get( 'set_date_newly' )."' )" : '')
   . ($this->params->get( 'set_date_older' ) ?                                 "\n  AND (TO_DAYS(ADDDATE(".$db->Quote($now).", INTERVAL $tzoffset HOUR)) - TO_DAYS(ADDDATE(".$this->created.", INTERVAL $tzoffset HOUR)) >= '".$this->params->get( 'set_date_older' )."' )" : '')

   . ($this->params->get( 'set_date_today') ?                                         "\n   AND (TO_DAYS(ADDDATE(".$db->Quote($now).", INTERVAL $tzoffset HOUR)) = TO_DAYS(ADDDATE(".$this->created.", INTERVAL $tzoffset HOUR)))" : '')
   . ($this->params->get( 'set_date_month') ?                                         "\n  AND (".$this->params->get( 'set_date_month')." = MONTH(ADDDATE(".$this->created.", INTERVAL $tzoffset HOUR)))" : '')
   . ($this->params->get( 'set_date_year') ?                                  "\n  AND (".$this->params->get( 'set_date_year')."  = YEAR(ADDDATE(".$this->created.", INTERVAL $tzoffset HOUR)))" : '')
